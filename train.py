...
# }
# PEFT_SUPPORT = ["LORA", "PREFIX_TUNING", "P_TUNING", "PROMPT_TUNING"]
...
def main() -> None:
    ...
    if not args.arg_paths:
        ...
        raw_datasets_split: datasets.DatasetDict = datasets.load_dataset(
            path=args.dataset.loader_path,
            cache_dir=args.dataset.data_store_path,
            name=args.dataset.name if args.dataset.name else None)
        ...
        for task, arg_path in args.arg_paths:
            ...
            logging.info('task_args.bert.location: {}'.format(task_args.bert.location))
            task_raw_datasets_split: datasets.DatasetDict = datasets.load_dataset(
                path=task_args.dataset.loader_path,
                cache_dir=task_args.dataset.data_store_path,
                name=task_args.dataset.name if task_args.dataset.name else None)
            logging.info("cache root: {}".format(cache_root))
    ...
    # PEFT
    # if args.peft:
    #     print("Use PEFT: {}".format(args.peft.peft_type))
    #     if args.peft.peft_type not in PEFT_SUPPORT:
    #         raise NotImplementedError("Unsupported PEFT type: {}".format(args.peft.peft_type))
    #     if args.peft.peft_type == "LORA":
    #         # https://huggingface.co/docs/peft/main/en/package_reference/lora
    #         # required: lora_r, lora_alpha, lora_dropout
    #         peft_config = LoraConfig(
    #             r=args.peft.lora_r,  # 16
    #             lora_alpha=args.peft.lora_alpha,  # 32
    #             target_modules=["q", "v"],
    #             lora_dropout=args.peft.lora_dropout,  # 0.05
    #             bias="none",
    #             task_type=TaskType.SEQ_2_SEQ_LM
    #         )
    #     elif args.peft.peft_type == "PREFIX_TUNING":
    #         # https://huggingface.co/docs/peft/main/en/package_reference/prefix_tuning
    #         peft_config = PrefixTuningConfig(
    #             peft_type="PREFIX_TUNING",
    #             task_type=TaskType.SEQ_2_SEQ_LM,
    #             num_virtual_tokens=args.peft.num_virtual_tokens,  # 20
    #             # token_dim=768,
    ...
    #     print("PEFT config: {}".format(peft_config))
    ...
    logger.info('Trainer build successfully.')
