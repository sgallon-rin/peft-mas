...
def count_num_parameters(model, only_trainable: bool = False, exclude_embeddings: bool = False) -> int:
    """
    adapted　from transformers.modeling_tools
...
class Model(PushToHubFriendlyModel):
    def __init__(self, args):
        ...
        self.prefix_dropout = args.prefix_tuning.prefix_dropout
        ...
        from ..prompt.modeling_mbart import MBartForConditionalGeneration
        from ..prompt.modeling_t5 import T5ForConditionalGeneration
        from ..prompt.modeling_mt5 import MT5ForConditionalGeneration
        if isinstance(self.pretrain_model, BartForConditionalGeneration) or isinstance(self.pretrain_model, MBartForConditionalGeneration):
        ...
        elif isinstance(self.pretrain_model, T5ForConditionalGeneration) or isinstance(self.pretrain_model, MT5ForConditionalGeneration):
        ...
        num_prefix_params = 0
        num_prefix_params += count_num_parameters(self.wte)
        num_prefix_params += count_num_parameters(self.control_trans)
        num_prefix_params += count_num_parameters(self.wte_dec)
        num_prefix_params += count_num_parameters(self.control_trans_dec)
        num_prefix_params += count_num_parameters(self.wte_enc)
        num_prefix_params += count_num_parameters(self.control_trans_enc)
        print("Prefixtuning; Pretrained Model num_parameters: {} / Prefix num_parameters: {} / Total: {}"
              .format(self.pretrain_model.num_parameters(), num_prefix_params,
                      self.pretrain_model.num_parameters() + num_prefix_params))
    ...
    def forward(self,
        ...
        past_prompt = self.get_prompt(
            bsz=bsz,
            # description=description_representation, knowledge=knowledge_representation,
        )
