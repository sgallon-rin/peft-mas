...
class TrainDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        ...
        cache_path = os.path.join(cache_root, "{}_xlsum_train.cache".format(self.lang))
...
class DevDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        self.raw_datasets = raw_datasets
        self.lang = args.seq2seq.lang
        cache_path = os.path.join(cache_root, "{}_xlsum_dev.cache".format(self.lang))
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.extended_data = torch.load(cache_path)
        else:
...
class TestDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        self.raw_datasets = raw_datasets
        self.lang = args.seq2seq.lang
        cache_path = os.path.join(cache_root, "{}_xlsum_test.cache".format(self.lang))
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.extended_data = torch.load(cache_path)
        else:
