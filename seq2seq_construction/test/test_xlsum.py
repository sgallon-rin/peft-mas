...
class TrainDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        ...
        cache_path = os.path.join(cache_root, "test_{}_xlsum_train.cache".format(self.lang))
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.extended_data = torch.load(cache_path)
        else:
            logger.info("Creating cache for xlsum_train of language {}".format(self.lang))
            self.extended_data = []
            ...
            self.extended_data = self.extended_data[:10]
...
class DevDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        self.raw_datasets = raw_datasets
        self.lang = args.seq2seq.lang
        cache_path = os.path.join(cache_root, "test_{}_xlsum_dev.cache".format(self.lang))
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.extended_data = torch.load(cache_path)
        else:
            logger.info("Creating cache for xlsum_dev of language {}".format(self.lang))
            self.extended_data = []
            expansion = args.seq2seq.expansion if args.seq2seq.expansion else 1
            for expand_id in range(expansion):
                for raw_data in tqdm(self.raw_datasets):
...
class TestDataset(Dataset):
    def __init__(self, args, raw_datasets, cache_root):
        self.raw_datasets = raw_datasets
        self.lang = args.seq2seq.lang
        cache_path = os.path.join(cache_root, "test_{}_xlsum_test.cache".format(self.lang))
        if os.path.exists(cache_path) and args.dataset.use_cache:
            self.extended_data = torch.load(cache_path)
        else:
            logger.info("Creating cache for xlsum_test of language {}".format(self.lang))
            self.extended_data = []
            expansion = args.seq2seq.expansion if args.seq2seq.expansion else 1
            for expand_id in range(expansion):
                for raw_data in tqdm(self.raw_datasets):
